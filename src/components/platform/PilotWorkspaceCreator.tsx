...
export function PilotWorkspaceCreator({ lang }: { lang: string }) {
  ...
  return <div dir={fa ? 'rtl' : 'ltr'}>
    ...
    {open && <form onSubmit={submit} className="mt-4 grid gap-3 rounded-xl border border-[#1E3A5A] bg-[#0F2236] p-5 text-sm text-white md:grid-cols-2">
      ...
      <p className="md:col-span-2 text-xs text-amber-200">{fa ? 'مشتری پیش‌نویس و محیط PILOT در LEAD ایجاد می‌شوند؛ ایمیل یا دعوت ارسال نمی‌شود.' : 'Creates a draft tenant and PILOT workspace in LEAD. No email or invitation is sent.'}</p>
