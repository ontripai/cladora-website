  ...
  return <section dir={fa ? 'rtl' : 'ltr'} className="mx-auto max-w-7xl space-y-5 text-white">
    ...
    <div className="grid gap-4">{requests.map(item => <article key={item.id} className="rounded-xl border border-[#1E3A5A] bg-[#0F2236] p-4 text-sm">
      <div className="flex flex-wrap items-center justify-between gap-2"><h2 className="font-bold">{item.full_name} · {item.reference_id}</h2>
        <span className="rounded bg-[#14324F] px-2 py-1">{item.applicant_type === 'multi_unit_owner' ? lang === 'fa' ? 'مالک چندواحدی' : lang === 'ro' ? 'Proprietar cu mai multe unități' : 'Multi-unit owner' : item.lead_type} · {item.status}</span></div>
      ...
      {item.requested_workspace_type && <div className="mt-2 rounded border border-[#1E3A5A] p-3 text-xs text-slate-200">
        <p>{item.applicant_type === 'multi_unit_owner' ? fa ? 'نوع غالب واحدها:' : lang === 'ro' ? 'Tipul predominant al unităților:' : 'Predominant unit type:' : fa ? 'نوع ورک‌اسپیس درخواستی:' : 'Requested workspace:'} {Object.hasOwn(PILOT_WORKSPACE_TYPES, item.requested_workspace_type) ? PILOT_WORKSPACE_TYPES[item.requested_workspace_type as PilotWorkspaceType][lang === 'fa' || lang === 'ro' ? lang : 'en'] : item.requested_workspace_type}{item.applicant_type !== 'multi_unit_owner' && <> · {fa ? 'تعداد:' : 'Count:'} {item.requested_workspace_count ?? 1}</>}</p>
        <p>{fa ? 'زیرنوع:' : 'Subtype:'} {item.requested_workspace_subtype || '—'} · {fa ? 'متقاضی:' : 'Applicant:'} {item.applicant_type === 'multi_unit_owner' ? lang === 'fa' ? 'مالک چندواحدی' : lang === 'ro' ? 'Proprietar cu mai multe unități' : 'Multi-unit owner' : item.applicant_type || '—'} · {fa ? 'واحد/فضا:' : 'Units/spaces:'} {item.units_count ?? '—'}</p>
