...
export function CustomerStaffResponsibilitiesPanel({ lang }: { lang: string }) {
  ...
  return <section dir={fa ? 'rtl' : 'ltr'} className="space-y-4 rounded-xl border border-[#1E3A5A] bg-[#0F2236] p-5 text-sm text-white">
    ...
    {form && <form onSubmit={submit} className="grid gap-3 rounded-lg border border-[#1E3A5A] p-4 md:grid-cols-2">
      ...
      <label>{label('Workspace','Spațiu de lucru','محیط کاری')}<select required name="workspace_id" className="mt-1 w-full rounded bg-[#081320] p-2"><option value="">—</option>{workspaces.map(w => <option key={w.id} value={w.id}>{w.tenant_legal_name ?? w.commercial_owner ?? w.id} · {w.id.slice(0, 8)}</option>)}</select></label>
      <label>{label('Employee','Angajat','کارمند')}<select required name="platform_user_id" className="mt-1 w-full rounded bg-[#081320] p-2"><option value="">—</option>{users.map(u => <option key={u.id} value={u.id}>{u.display_name} · {u.employee_ref}</option>)}</select></label>
