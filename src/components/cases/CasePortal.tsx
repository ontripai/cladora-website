  ...
  return <main dir={fa?'rtl':'ltr'} className="mx-auto min-h-screen max-w-4xl space-y-5 bg-[#F6F9FC] px-5 pb-20 pt-28 text-[#102A43]">
    ...
    {manager&&preparedItems.length>0&&<section className="space-y-3 rounded-xl border bg-white p-4"><h2 className="font-bold">{fa?'ورک‌اسپیس‌های آماده‌شده برای این مشتری':'Workspaces prepared for this customer'}</h2>
      {preparedItems.map(item=><div key={item.workspace_id} className="space-y-2 border-t pt-3 text-sm">
        ...
        {!item.linked&&!item.approval_ready&&['OWNER_PORTFOLIO','HYBRID'].includes(item.workspace_type)&&<p className="text-amber-800">{fa?'تأیید دسترسی پس از تعریف نقش مدیر اصلی متناسب با این ساختار فعال می‌شود.':'Access approval will open after the correct primary administrator role is defined.'}</p>}
        {!item.linked&&!item.approval_ready&&approver&&!['OWNER_PORTFOLIO','HYBRID'].includes(item.workspace_type)&&<button type="button" onClick={()=>setApprovalItem(item)} className="rounded border border-amber-600 px-3 py-2 text-amber-900">{fa?'ثبت مجوز آزمایشی یا قراردادی':'Record pilot or contract approval'}</button>}
