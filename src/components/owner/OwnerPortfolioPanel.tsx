...
type View = { units: Unit[]; count: number; leases: Lease[]; entries: Entry[]; lease_count?: number; entry_count?: number };
type LoadedView = { unitId: string | null; offset: number; detailsOffset: number; data: View };
...
export function OwnerPortfolioPanel({ lang }: { lang: Language }) {
  ...
  const detailsReady = loadedView?.unitId === selected && loadedView?.offset === offset && loadedView?.detailsOffset === detailsOffset;
  const view: View = {
    lease_count: loadedView?.data.lease_count, entry_count: loadedView?.data.entry_count,
  ...
  async function recordPayment(event: React.FormEvent<HTMLFormElement>, entryId: string) {
    event.preventDefault(); setBusy(true); setError('');
    const paid_on = new FormData(event.currentTarget).get('paid_on');
    try {
      const response = await fetch(`${endpoint}/cash`, {method:'PATCH',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify({entry_id:entryId,paid_on})});
  ...
  return <main dir={lang === 'fa' ? 'rtl' : 'ltr'} className="mx-auto max-w-5xl space-y-6 p-6 text-slate-900">
    ...
    <form onSubmit={event => void submit(event,'unit')} className="grid gap-3 rounded-xl border bg-white p-5 sm:grid-cols-2">
      ...
      <label>{t.usage}<select name="usage_kind" className="mt-1 w-full rounded border p-2"><option value="residential">{label('residential')}</option><option value="commercial">{label('commercial')}</option><option value="office">{label('office')}</option><option value="industrial">{label('industrial')}</option><option value="other">{label('other')}</option></select></label>
    ...
    <section className="rounded-xl border bg-white p-5"><h2 className="font-bold">{t.unit} · {view.count}</h2>
      ...
      <ul className="mt-3 grid gap-2 sm:grid-cols-2">{view.units.map(unit => <li key={unit.id}><button type="button" disabled={busy} onClick={() => { setError(''); setDetailsOffset(0); setSelected(unit.id); }} className={`w-full rounded border p-3 text-start ${selected === unit.id ? 'border-teal-700 bg-teal-50' : ''}`}><strong>{unit.building_label} · {unit.unit_label}</strong><span className="block text-sm text-slate-600">{unit.address_text} · {label(unit.usage_kind)}</span></button></li>)}</ul>
    ...
    {selected && <Fragment key={`${selected}:${detailsOffset}`}><fieldset disabled={busy || !detailsReady} aria-busy={!detailsReady} className="space-y-6">
      ...
      <p className="text-sm text-slate-600">{lang === 'fa' ? 'شناسه ورک‌اسپیس و واحد رسمی را از مدیر ساختمان دریافت کنید. درخواست پس از بررسی مدیر و تأیید سوپرادمین فعال می‌شود؛ این اتصال دسترسی کلی به ساختمان نمی‌دهد.' : lang === 'ro' ? 'Solicită administratorului identificatorii spațiului și unității. Cererea necesită verificarea administratorului și aprobarea platformei.' : 'Obtain the workspace and official unit IDs from the building manager. The manager and platform administrator must verify the request.'}</p>
      <form className="grid gap-2 sm:grid-cols-2" onSubmit={event => { event.preventDefault(); const form = event.currentTarget; const fields = Object.fromEntries(new FormData(form)); void linkAction({ action: 'request', private_unit_id: selected, workspace_id: fields.workspace_id, canonical_unit_id: fields.canonical_unit_id, evidence: fields.evidence }, form); }}>
        <label>{lang === 'fa' ? 'شناسه محیط ساختمان' : lang === 'ro' ? 'Identificator spațiu' : 'Workspace ID'}<input name="workspace_id" required className="w-full rounded border p-2" /></label>
        <label>{lang === 'fa' ? 'شناسه واحد رسمی' : lang === 'ro' ? 'Identificator unitate oficială' : 'Official unit ID'}<input name="canonical_unit_id" required className="w-full rounded border p-2" /></label>
        <label className="sm:col-span-2">{lang === 'fa' ? 'مرجع مدرک مالکیت یا توضیح قابل بررسی' : lang === 'ro' ? 'Referință dovadă de proprietate' : 'Ownership evidence reference'}<textarea name="evidence" required minLength={15} maxLength={500} className="w-full rounded border p-2" /></label>
        <button disabled={busy} className="rounded bg-teal-700 p-2 text-white disabled:opacity-50">{lang === 'fa' ? 'درخواست اتصال' : lang === 'ro' ? 'Solicită conectarea' : 'Request connection'}</button>
      </form>
      <ul className="space-y-2">{links.filter(link => link.private_unit_id === selected).map(link => <li key={link.id} className="flex flex-wrap items-center justify-between gap-2 rounded border p-2"><span>{label(link.status)} · {link.workspace_id} · {link.canonical_unit_id}</span>{['requested','manager_verified','linked'].includes(link.status) && <button type="button" disabled={busy} className="rounded border px-2 py-1" onClick={() => void linkAction({ action: 'withdraw', link_id: link.id })}>{lang === 'fa' ? 'لغو اتصال' : lang === 'ro' ? 'Retrage' : 'Withdraw'}</button>}</li>)}</ul>
      ...
      <p className="text-sm text-slate-600">{lang === 'fa' ? 'فقط صورتحساب‌های صادرشده برای مالک تأییدشده نمایش داده می‌شود. این بخش جدا از ثبت‌های مالی شخصی شماست.' : lang === 'ro' ? 'Facturile emise pentru proprietarul verificat sunt separate de evidențele private.' : 'Issued invoices owed by the verified owner are shown separately from self-reported cash records.'}</p>
      {charges.length === 0 && <p className="text-sm text-slate-500">{lang === 'fa' ? 'صورتحساب قابل نمایشی وجود ندارد.' : lang === 'ro' ? 'Nu există facturi oficiale de afișat.' : 'No visible official invoices.'}</p>}
      <ul className="space-y-2">{charges.map(charge => <li key={charge.id} className="rounded border p-2">#{charge.invoice_no} · {charge.total} {charge.currency} · {label(charge.status)} · {lang === 'fa' ? 'باقی‌مانده' : lang === 'ro' ? 'Rest de plată' : 'Outstanding'}: {charge.outstanding_amount ?? '—'} · {charge.due_on ?? '—'}</li>)}</ul>
      ...
      <form onSubmit={event=>void submit(event,'lease')} className="grid gap-2 sm:grid-cols-2">
        ...
        <select name="currency" aria-label={lang === 'fa' ? 'ارز' : lang === 'ro' ? 'Monedă' : 'Currency'} className="rounded border p-2"><option>RON</option><option>EUR</option><option>USD</option></select>
        <button disabled={busy} className="rounded bg-teal-700 p-2 text-white disabled:opacity-50">{t.recordLease}</button>
      </form>
      <ul className="space-y-2">{view.leases.map(l => <li key={l.id} className="rounded border p-2">{l.tenant_label} · {l.starts_on} – {l.ends_on ?? '…'} · {l.monthly_rent} {l.currency} · {label(l.status)}
        <div className="mt-2 flex gap-2">{l.status === 'draft' && <><button type="button" disabled={busy} onClick={() => void changeLease(l.id,'activate')} className="rounded border px-2 py-1">{lang === 'fa' ? 'فعال‌سازی' : lang === 'ro' ? 'Activează' : 'Activate'}</button><button type="button" disabled={busy} onClick={() => void changeLease(l.id,'cancel')} className="rounded border px-2 py-1">{lang === 'fa' ? 'لغو پیش‌نویس' : lang === 'ro' ? 'Anulează proiectul' : 'Cancel draft'}</button></>}{l.status === 'active' && <button type="button" disabled={busy} onClick={() => void changeLease(l.id,'end')} className="rounded border px-2 py-1">{lang === 'fa' ? 'پایان قرارداد' : lang === 'ro' ? 'Încheie contractul' : 'End lease'}</button>}</div>
      </li>)}</ul>
      ...
      <form onSubmit={event=>void submit(event,'cash')} className="grid gap-2 sm:grid-cols-2">
        <label>{t.kind}<select name="kind" className="w-full rounded border p-2"><option value="rent">{label('rent')}</option><option value="building_charge">{label('building_charge')}</option><option value="owner_expense">{label('owner_expense')}</option><option value="tax_reserve">{label('tax_reserve')}</option><option value="other">{label('other')}</option></select></label>
        <select name="direction" aria-label={lang === 'fa' ? 'جهت' : lang === 'ro' ? 'Direcție' : 'Direction'} className="rounded border p-2"><option value="income">{label('income')}</option><option value="expense">{label('expense')}</option></select>
        ...
        <label>{lang === 'fa' ? 'قرارداد مرتبط با اجاره دریافتی (اختیاری)' : lang === 'ro' ? 'Contract asociat încasării (opțional)' : 'Related rent lease (optional)'}<select name="lease_id" className="w-full rounded border p-2"><option value="">—</option>{view.leases.filter(l => l.status === 'active' || l.status === 'ended').map(l => <option key={l.id} value={l.id}>{l.tenant_label} · {l.starts_on}</option>)}</select></label>
        <button disabled={busy} className="rounded bg-teal-700 p-2 text-white disabled:opacity-50">{t.recordCash}</button>
      </form><p className="text-xs text-slate-500">{t.sourced} · {lang === 'fa' ? 'برای ثبت دریافت مرتبط با قرارداد، دستهٔ اجاره و جهت درآمد و هر دو تاریخ سررسید و دریافت را وارد کنید.' : lang === 'ro' ? 'Pentru o încasare asociată unui contract, selectează Chirie și Încasare și introdu scadența și data încasării.' : 'For a lease payment, select Rent and Income and enter both due and received dates.'}</p>
      <ul className="space-y-2">{view.entries.map(e => <li key={e.id} className="rounded border p-2">{label(e.kind)} · {label(e.direction)} · {e.amount} {e.currency} · {e.due_on ?? '—'} · {e.paid_on ?? '—'}{e.lease_id && ` · ${lang === 'fa' ? 'قرارداد' : lang === 'ro' ? 'Contract' : 'Lease'} ${e.lease_id}`}{!e.paid_on && <form onSubmit={event=>void recordPayment(event,e.id)} className="mt-2 flex flex-wrap items-end gap-2"><label className="text-sm">{t.paid}<input required name="paid_on" type="date" className="block max-w-full rounded border p-2" /></label><button disabled={busy} className="rounded bg-teal-700 p-2 text-sm text-white disabled:opacity-50">{lang==='fa'?'ثبت پرداخت کامل':lang==='ro'?'Înregistrează plata integrală':'Record full payment'}</button></form>}</li>)}</ul>
    </section></fieldset><nav aria-label={lang === 'fa' ? 'صفحات سوابق واحد' : lang === 'ro' ? 'Paginile evidențelor unității' : 'Unit record pages'} className="flex flex-wrap gap-3">
      <button type="button" disabled={busy || detailsOffset===0} onClick={()=>setDetailsOffset(Math.max(0,detailsOffset-100))} className="rounded border p-2 disabled:opacity-50">{t.previous}</button>
      <span className="p-2">{lang === 'fa' ? 'سوابق واحد' : lang === 'ro' ? 'Evidențe unitate' : 'Unit records'} · {Math.floor(detailsOffset/100)+1}</span>
      <button type="button" disabled={busy || !detailsReady || detailsOffset+100>=Math.max(view.lease_count??view.leases.length,view.entry_count??view.entries.length)} onClick={()=>setDetailsOffset(detailsOffset+100)} className="rounded border p-2 disabled:opacity-50">{t.next}</button>
    </nav></Fragment>}
  </main>;
}
