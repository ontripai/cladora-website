...
import { ocrCandidateSchema } from "@/lib/customer/utilities-schema";
...
export async function POST(request: NextRequest) {
  ...
  const parsed = ocrCandidateSchema.safeParse(bodyJson);
  if (!parsed.success) {
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("create_ocr_candidate_v1", {
    ...
    p_photo_object_path: p.photo_object_path ?? null,
