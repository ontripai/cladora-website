...
import { captureReadingSchema } from "@/lib/customer/utilities-schema";
...
export async function POST(request: NextRequest) {
  ...
  const parsed = captureReadingSchema.safeParse(bodyJson);
  if (!parsed.success) {
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("capture_reading_v1", {
    p_context_id: p.context_id,
    p_meter_id: p.meter_id,
    p_reading_value: p.reading_value,
    p_reading_at: p.reading_at ?? new Date().toISOString(),
    p_method: p.method,
    p_source_object_path: p.source_object_path ?? null,
    p_note: p.note ?? null,
