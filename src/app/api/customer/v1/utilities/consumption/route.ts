...
import { calculateConsumptionSchema } from "@/lib/customer/utilities-schema";
...
export async function POST(request: NextRequest) {
  ...
  const parsed = calculateConsumptionSchema.safeParse(bodyJson);
  if (!parsed.success) {
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("calculate_consumption_v1", {
    p_context_id: p.context_id,
    p_meter_id: p.meter_id,
    p_start_reading_id: p.start_reading_id,
    p_end_reading_id: p.end_reading_id,
  });
