...
import { decommissionMeterSchema } from "@/lib/customer/utilities-schema";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const parsed = decommissionMeterSchema.safeParse(bodyJson);
  if (!parsed.success) {
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("decommission_meter_v1", {
    p_context_id: p.context_id,
    p_meter_id: id,
    p_final_reading: p.final_reading ?? null,
    p_decommissioned_on: p.decommissioned_on ?? null,
    p_reason: p.reason,
  });
