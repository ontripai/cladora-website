...
import { replaceMeterSchema } from "@/lib/customer/utilities-schema";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const parsed = replaceMeterSchema.safeParse(bodyJson);
  if (!parsed.success) {
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("replace_meter_v1", {
    p_context_id: p.context_id,
    p_old_meter_id: id,
    p_final_reading: p.final_reading,
    p_new_serial_number: p.new_serial_number,
    p_new_initial_reading: p.new_initial_reading,
    p_replaced_at: p.replaced_at ?? null,
    p_reason: p.reason,
  });
