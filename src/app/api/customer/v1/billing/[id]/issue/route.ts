...
import { issueBillRequestSchema } from "@/lib/customer/billing-schema";
...
export async function POST(
  ...
  const parsed = issueBillRequestSchema.safeParse(rawBody);
  if (!parsed.success) {
  ...
  const b = parsed.data;
  const { data, error: rpcError } = await supabase.schema("customer_api").rpc("issue_bill_v1", {
    p_context_id: b.context_id,
    p_invoice_id: id,
    p_issued_on: b.issued_on ?? null,
