...
import { submitQuoteSchema } from "@/lib/customer/maintenance-schema";
...
export async function POST(request: NextRequest) {
  ...
  const parsed = submitQuoteSchema.safeParse(bodyJson);
  if (!parsed.success) {
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("submit_quote_v1", {
    p_context_id: p.context_id,
    p_work_order_id: p.work_order_id,
    p_vendor_id: p.vendor_id,
    p_quote_ref: p.quote_ref,
    p_subtotal: p.subtotal,
    p_tax_total: p.tax_total,
    p_currency: p.currency,
    p_valid_until: p.valid_until ?? null,
    p_scope_snapshot: p.scope_snapshot ?? {},
