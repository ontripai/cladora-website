...
import { selectQuoteSchema } from "@/lib/customer/maintenance-schema";
...
export async function POST(
  ...
  const parsed = selectQuoteSchema.safeParse(bodyJson);
  if (!parsed.success) {
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("select_quote_v1", {
    p_context_id: p.context_id,
    p_quote_id: quoteId,
    p_selection_reason: p.selection_reason,
  });
