...
import { createRfqSchema } from "@/lib/customer/maintenance-schema";
...
export async function POST(request: NextRequest) {
  ...
  const parsed = createRfqSchema.safeParse(bodyJson);
  if (!parsed.success) {
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("create_rfq_v1", {
    p_context_id: p.context_id,
    p_work_order_id: p.work_order_id ?? null,
    p_ticket_id: p.ticket_id ?? null,
    p_title: p.title,
    p_scope_description: p.scope_description ?? null,
    p_due_date: p.due_date,
    p_invited_vendor_ids: p.invited_vendor_ids,
  });
