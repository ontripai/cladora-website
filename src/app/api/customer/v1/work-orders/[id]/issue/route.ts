...
import { issueWorkOrderSchema } from "@/lib/customer/maintenance-schema";
...
export async function POST(
  ...
  const parsed = issueWorkOrderSchema.safeParse(bodyJson);
  if (!parsed.success) {
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("issue_work_order_v1", {
