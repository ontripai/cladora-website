...
import { completeWorkOrderSchema } from "@/lib/customer/maintenance-schema";
...
export async function POST(
  ...
  const parsed = completeWorkOrderSchema.safeParse(bodyJson);
  if (!parsed.success) {
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("complete_work_order_v1", {
    p_context_id: p.context_id,
    p_work_order_id: workOrderId,
    p_completion_notes: p.completion_notes,
    p_actual_cost: p.actual_cost ?? null,
  });
