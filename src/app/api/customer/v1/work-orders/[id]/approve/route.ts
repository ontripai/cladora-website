...
import { approveWorkOrderSchema } from "@/lib/customer/maintenance-schema";
...
export async function POST(
  ...
  const parsed = approveWorkOrderSchema.safeParse(bodyJson);
  if (!parsed.success) {
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("approve_work_order_v1", {
    p_context_id: p.context_id,
    p_work_order_id: workOrderId,
    p_approved_budget: p.approved_budget ?? null,
