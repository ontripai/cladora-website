...
import { verifyWorkOrderSchema } from "@/lib/customer/maintenance-schema";
...
export async function POST(
  ...
  const parsed = verifyWorkOrderSchema.safeParse(bodyJson);
  if (!parsed.success) {
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("verify_work_order_v1", {
    p_context_id: p.context_id,
    p_work_order_id: workOrderId,
    p_verification_notes: p.verification_notes,
  });
