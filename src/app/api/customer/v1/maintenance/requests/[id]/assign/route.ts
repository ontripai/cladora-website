...
import { assignRequestSchema } from "@/lib/customer/maintenance-schema";
...
export async function POST(
  ...
  const parsed = assignRequestSchema.safeParse(bodyJson);
  if (!parsed.success) {
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("assign_maintenance_request_v1", {
