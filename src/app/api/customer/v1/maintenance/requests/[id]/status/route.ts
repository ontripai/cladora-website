...
import { changeRequestStatusSchema } from "@/lib/customer/maintenance-schema";
...
export async function POST(
  ...
  const parsed = changeRequestStatusSchema.safeParse(bodyJson);
  if (!parsed.success) {
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("change_maintenance_status_v1", {
    p_context_id: p.context_id,
    p_ticket_id: ticketId,
    p_new_status: p.status,
    p_reason: p.reason ?? null,
    p_resolution_summary: p.resolution_summary ?? null,
  });
