...
import { reopenRequestSchema } from "@/lib/customer/maintenance-schema";
...
export async function POST(
  ...
  const parsed = reopenRequestSchema.safeParse(bodyJson);
  if (!parsed.success) {
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("reopen_maintenance_request_v1", {
