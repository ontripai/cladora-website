...
import { createMaintenanceRequestSchema } from "@/lib/customer/maintenance-schema";
...
export async function GET(request: NextRequest) {
  ...
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("list_maintenance_requests_v1", {
...
export async function POST(request: NextRequest) {
  ...
  const parsed = createMaintenanceRequestSchema.safeParse(bodyJson);
  if (!parsed.success) {
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("create_maintenance_request_v1", {
    ...
    p_safety_impact: p.safety_impact,
    p_access_instructions: p.access_instructions ?? null,
    p_preferred_window: p.preferred_window ?? null,
    p_source: p.source,
  });
