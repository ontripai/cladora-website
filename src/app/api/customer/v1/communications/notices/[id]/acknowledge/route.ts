...
import { acknowledgeNoticeSchema } from "@/lib/customer/communications-schema";
...
export async function POST(
  ...
  const parsed = acknowledgeNoticeSchema.safeParse(bodyJson);
  if (!parsed.success) {
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("acknowledge_notice_v1", {
