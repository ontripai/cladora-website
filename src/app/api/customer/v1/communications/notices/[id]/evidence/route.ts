...
import { recordStatutoryEvidenceSchema } from "@/lib/customer/communications-schema";
...
export async function POST(
  ...
  const parsed = recordStatutoryEvidenceSchema.safeParse(bodyJson);
  if (!parsed.success) {
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("record_statutory_evidence_v1", {
    p_context_id: p.context_id,
    p_notice_id: noticeId,
    p_evidence_type: p.evidence_type,
    p_evidence_reference: p.evidence_reference,
    p_occurred_at: p.occurred_at ?? null,
    p_checksum: p.checksum ?? null,
