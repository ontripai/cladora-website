...
import { cancelNoticeSchema } from "@/lib/customer/communications-schema";
...
export async function POST(
  ...
  const parsed = cancelNoticeSchema.safeParse(bodyJson);
  if (!parsed.success) {
  ...
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("cancel_notice_v1", {
