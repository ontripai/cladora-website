...
import { approveNoticeSchema } from "@/lib/customer/communications-schema";
...
export async function POST(
  ...
  const parsed = approveNoticeSchema.safeParse(bodyJson);
  if (!parsed.success) {
  ...
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("approve_notice_v1", {
