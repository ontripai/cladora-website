...
import { verifyStatutoryEvidenceSchema } from "@/lib/customer/communications-schema";
...
export async function POST(
  ...
  const parsed = verifyStatutoryEvidenceSchema.safeParse(bodyJson);
  if (!parsed.success) {
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("verify_statutory_evidence_v1", {
    p_context_id: p.context_id,
    p_evidence_id: evidenceId,
    p_decision: p.decision,
    p_rejection_reason: p.rejection_reason ?? null,
  });
