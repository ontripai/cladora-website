...
import { resolveClaimSchema } from "@/lib/customer/assets-schema";
...
export async function POST(
  ...
  const parsed = resolveClaimSchema.safeParse(bodyJson);
  if (!parsed.success) {
    return NextResponse.json(
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("resolve_warranty_claim_v1", {
    p_context_id: p.context_id,
    p_claim_id: claimId,
    p_resolution_status: p.resolution_status,
    p_resolution_notes: p.resolution_notes ?? null,
  });
