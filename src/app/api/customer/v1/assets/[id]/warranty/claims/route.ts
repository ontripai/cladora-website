...
import { assetDetailQuerySchema, createClaimSchema } from "@/lib/customer/assets-schema";
...
export async function GET(
  ...
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("list_warranty_claims_v1", {
...
export async function POST(
  ...
  const parsed = createClaimSchema.safeParse(bodyJson);
  if (!parsed.success) {
    return NextResponse.json(
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("create_warranty_claim_v1", {
    p_context_id: p.context_id,
    p_warranty_id: p.warranty_id,
    p_claim_reference: p.claim_reference,
    p_description: p.description,
    p_claim_date: p.claim_date ?? new Date().toISOString().slice(0, 10),
