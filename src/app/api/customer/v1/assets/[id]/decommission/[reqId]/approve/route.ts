...
import { approveDecommissionSchema } from "@/lib/customer/assets-schema";
...
export async function POST(
  ...
  const parsed = approveDecommissionSchema.safeParse(bodyJson);
  if (!parsed.success) {
    return NextResponse.json(
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("approve_asset_decommission_v1", {
