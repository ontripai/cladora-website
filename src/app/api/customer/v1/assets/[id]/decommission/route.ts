...
import { requestDecommissionSchema } from "@/lib/customer/assets-schema";
...
export async function POST(
  ...
  const parsed = requestDecommissionSchema.safeParse(bodyJson);
  if (!parsed.success) {
    return NextResponse.json(
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("request_asset_decommission_v1", {
    p_context_id: p.context_id,
    p_asset_id: assetId,
    p_reason: p.reason,
    p_replacement_asset_id: p.replacement_asset_id ?? null,
  });
