...
import { updateOperationalStatusSchema } from "@/lib/customer/assets-schema";
...
export async function PATCH(
  ...
  const parsed = updateOperationalStatusSchema.safeParse(bodyJson);
  if (!parsed.success) {
    return NextResponse.json(
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("update_asset_operational_status_v1", {
    p_context_id: p.context_id,
    p_asset_id: assetId,
    p_operational_status: p.operational_status,
