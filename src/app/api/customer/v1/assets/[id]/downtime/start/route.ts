...
import { startDowntimeSchema } from "@/lib/customer/assets-schema";
...
export async function POST(
  ...
  const parsed = startDowntimeSchema.safeParse(bodyJson);
  if (!parsed.success) {
    return NextResponse.json(
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("start_asset_downtime_v1", {
    p_context_id: p.context_id,
    p_asset_id: assetId,
    p_reason: p.reason,
    p_is_planned: p.is_planned,
    p_work_order_id: p.work_order_id ?? null,
    p_ticket_id: p.ticket_id ?? null,
    p_started_at: p.started_at ?? null,
  });
