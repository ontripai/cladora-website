...
import { verifyInspectionSchema } from "@/lib/customer/assets-schema";
import { hasTrustedMutationOrigin } from "@/lib/security/same-origin";
import { isApplicationJson, parseJsonWithLimit } from "@/lib/security/request-body";

export async function POST(
  request: NextRequest,
  context: { params: Promise<{ id: string; inspId: string }> }
) {
  ...
  const { inspId } = await context.params;
  ...
  const parsed = verifyInspectionSchema.safeParse(bodyJson);
  if (!parsed.success) {
    return NextResponse.json(
  ...
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("verify_asset_inspection_v1", {
    p_context_id: parsed.data.context_id,
    p_inspection_id: inspId,
  });
