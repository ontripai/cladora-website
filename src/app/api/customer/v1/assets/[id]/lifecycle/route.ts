...
import { transitionLifecycleSchema } from "@/lib/customer/assets-schema";
...
export async function POST(
  ...
  const parsed = transitionLifecycleSchema.safeParse(bodyJson);
  if (!parsed.success) {
    return NextResponse.json(
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("transition_asset_lifecycle_v1", {
    p_context_id: p.context_id,
    p_asset_id: assetId,
    p_target_status: p.target_status,
