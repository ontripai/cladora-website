...
import { attachWorkspaceRolePermissionRequestSchema } from '@/lib/customer/workspace-roles-schema';
...
export async function POST(request: NextRequest) {
  ...
  const parsed = attachWorkspaceRolePermissionRequestSchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json(
      {
        error: {
  ...
  const { data, error: rpcError } = await (supabase.schema('customer_api') as any)
    .rpc('attach_workspace_role_permission_v1', {
  ...
  if (rpcError) {
    ...
    if (msg.includes('permission_not_assignable_to_role_modules')) {
      return NextResponse.json(
        { error: { code: 'PERMISSION_NOT_ASSIGNABLE', message: 'Permission is not mapped to any module attached to this role.' } },
