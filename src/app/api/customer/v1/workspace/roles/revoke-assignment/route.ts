...
import { revokeWorkspaceRoleAssignmentRequestSchema } from '@/lib/customer/workspace-roles-schema';
...
export async function POST(request: NextRequest) {
  ...
  if (rpcError) {
    ...
    if (rpcError.code === '40001' || msg.includes('workspace_member_role_expected_lock_version_conflict')) {
      return NextResponse.json(
        { error: { code: 'LOCK_VERSION_CONFLICT', message: 'Assignment was modified concurrently. Please reload.' } },
      ...
      return NextResponse.json(
        {
          error: {
            code: 'MFA_REQUIRED',
            message: 'Multi-Factor Authentication (AAL2) is required to revoke assignments.',
            redirect_to: '/mfa',
          },
        },
        { status: 403, headers: HEADERS }
    ...
    if (msg.includes('workspace_member_role_already_revoked')) {
      return NextResponse.json(
        { error: { code: 'ALREADY_REVOKED', message: 'Assignment has already been revoked.' } },
        { status: 409, headers: HEADERS }
      );
    }
    return NextResponse.json(
