...
import { attachWorkspaceRoleModuleRequestSchema } from '@/lib/customer/workspace-roles-schema';
...
export async function POST(request: NextRequest) {
  ...
  const parsed = attachWorkspaceRoleModuleRequestSchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json(
      {
        error: {
  ...
  const { data, error: rpcError } = await (supabase.schema('customer_api') as any)
    .rpc('attach_workspace_role_module_v1', {
  ...
  if (rpcError) {
    ...
    if (msg.includes('workspace_module_not_active_in_workspace')) {
      return NextResponse.json(
        { error: { code: 'MODULE_NOT_ACTIVE', message: 'Module must be actively installed in workspace' } },
