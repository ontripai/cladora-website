...
import { detachWorkspaceRoleModuleRequestSchema } from '@/lib/customer/workspace-roles-schema';
...
export async function POST(request: NextRequest) {
  ...
  const parsed = detachWorkspaceRoleModuleRequestSchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json(
      {
        error: {
  ...
  const { data, error: rpcError } = await (supabase.schema('customer_api') as any)
    .rpc('detach_workspace_role_module_v1', {
