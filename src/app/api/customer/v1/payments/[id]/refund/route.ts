...
import { reversePaymentRequestSchema } from "@/lib/customer/payments-schema";
...
export async function POST(
  ...
  const parsed = reversePaymentRequestSchema.safeParse(rawBody);
  if (!parsed.success) {
  ...
  const b = parsed.data;
  const { data, error: rpcError } = await supabase.schema("customer_api").rpc("reverse_payment_v1", {
