...
import { unallocatePaymentRequestSchema } from "@/lib/customer/payments-schema";
...
export async function POST(
  ...
  const parsed = unallocatePaymentRequestSchema.safeParse(rawBody);
  if (!parsed.success) {
  ...
  const b = parsed.data;
  const { data, error: rpcError } = await supabase.schema("customer_api").rpc("unallocate_payment_v1", {
