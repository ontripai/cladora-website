...
import { allocatePaymentRequestSchema } from "@/lib/customer/payments-schema";
...
export async function POST(
  ...
  const parsed = allocatePaymentRequestSchema.safeParse(rawBody);
  if (!parsed.success) {
  ...
  const b = parsed.data;
  const { data, error: rpcError } = await supabase.schema("customer_api").rpc("allocate_payment_v1", {
    p_context_id: b.context_id,
    p_payment_id: id,
    p_allocations: b.allocations,
