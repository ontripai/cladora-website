...
import { unmatchBankTransactionRequestSchema } from "@/lib/customer/payments-schema";
...
export async function POST(
  ...
  const parsed = unmatchBankTransactionRequestSchema.safeParse(rawBody);
  if (!parsed.success) {
  ...
  const b = parsed.data;
  const { data, error: rpcError } = await supabase.schema("customer_api").rpc("unmatch_bank_transaction_v1", {
