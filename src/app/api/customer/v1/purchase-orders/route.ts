...
import { createPurchaseOrderSchema } from "@/lib/customer/maintenance-schema";
...
export async function POST(request: NextRequest) {
  ...
  const parsed = createPurchaseOrderSchema.safeParse(bodyJson);
  if (!parsed.success) {
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("create_purchase_order_v1", {
    ...
    p_payment_terms: p.payment_terms,
  });
