...
import { createGovernancePolicySchema } from "@/lib/customer/governance-schema";
...
export async function POST(request: NextRequest) {
  ...
  const parsed = createGovernancePolicySchema.safeParse(bodyJson);
  if (!parsed.success) {
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("create_governance_policy_v1", {
    p_context_id: p.context_id,
    p_property_id: p.property_id ?? null,
    p_name: p.name,
    p_voting_basis: p.voting_basis,
    p_notice_period_days: p.notice_period_days,
    p_quorum_threshold: p.quorum_threshold,
  });
