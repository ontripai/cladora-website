...
import { openBallotSchema } from "@/lib/customer/governance-schema";
...
export async function POST(
  ...
  const parsed = openBallotSchema.safeParse(bodyJson);
  if (!parsed.success) {
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("open_ballot_v1", {
    p_context_id: p.context_id,
    p_agenda_item_id: agendaItemId,
    p_question: p.question,
    p_secret_ballot: p.secret_ballot,
  });
