...
import { castVoteSchema } from "@/lib/customer/governance-schema";
...
export async function POST(
  ...
  const { id: ballotId } = await params;
  ...
  const parsed = castVoteSchema.safeParse(bodyJson);
  if (!parsed.success) {
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("cast_vote_v1", {
