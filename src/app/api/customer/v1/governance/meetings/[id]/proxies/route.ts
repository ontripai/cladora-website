...
import { registerProxySchema } from "@/lib/customer/governance-schema";
...
export async function POST(
  ...
  const parsed = registerProxySchema.safeParse(bodyJson);
  if (!parsed.success) {
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("register_proxy_v1", {
    p_context_id: p.context_id,
    p_meeting_id: meetingId,
    p_eligibility_id: p.eligibility_id,
    p_representative_party_id: p.representative_party_id,
    p_valid_from: p.valid_from,
    p_valid_until: p.valid_until,
  });
