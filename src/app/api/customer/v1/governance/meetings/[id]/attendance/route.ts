...
import { registerAttendanceSchema } from "@/lib/customer/governance-schema";
...
export async function POST(
  ...
  const parsed = registerAttendanceSchema.safeParse(bodyJson);
  if (!parsed.success) {
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("register_attendance_v1", {
    p_context_id: p.context_id,
    p_meeting_id: meetingId,
    p_eligibility_id: p.eligibility_id,
    p_attendance_mode: p.attendance_mode,
    p_represented_by_party_id: p.represented_by_party_id ?? null,
  });
