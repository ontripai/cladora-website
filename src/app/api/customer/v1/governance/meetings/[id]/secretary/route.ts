...
import { electMeetingSecretarySchema } from "@/lib/customer/governance-schema";
...
export async function POST(
  ...
  const parsed = electMeetingSecretarySchema.safeParse(bodyJson);
  if (!parsed.success) {
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("elect_meeting_secretary_v1", {
    p_context_id: p.context_id,
    p_meeting_id: meetingId,
    p_secretary_party_id: p.secretary_party_id,
    p_secretary_name: p.secretary_name,
  });
