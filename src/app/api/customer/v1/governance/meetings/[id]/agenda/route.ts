...
import { addAgendaItemSchema } from "@/lib/customer/governance-schema";
...
export async function POST(
  ...
  const parsed = addAgendaItemSchema.safeParse(bodyJson);
  if (!parsed.success) {
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("add_agenda_item_v1", {
    p_context_id: p.context_id,
    p_meeting_id: meetingId,
    p_sequence_no: p.sequence_no,
    p_title: p.title,
    p_description: p.description ?? null,
    p_item_type: p.item_type,
    p_voting_required: p.voting_required,
    p_proposed_amount: p.proposed_amount ?? null,
