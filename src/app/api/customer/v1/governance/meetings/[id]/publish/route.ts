...
import { publishMeetingSchema } from "@/lib/customer/governance-schema";
...
export async function POST(
  ...
  const parsed = publishMeetingSchema.safeParse(bodyJson);
  if (!parsed.success) {
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("publish_meeting_v1", {
