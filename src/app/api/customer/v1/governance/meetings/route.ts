...
import { createMeetingSchema } from "@/lib/customer/governance-schema";
...
export async function POST(request: NextRequest) {
  ...
  const parsed = createMeetingSchema.safeParse(bodyJson);
  if (!parsed.success) {
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("create_meeting_v1", {
    p_context_id: p.context_id,
    p_property_id: p.property_id,
    p_title: p.title,
    p_meeting_type: p.meeting_type,
    p_scheduled_at: p.scheduled_at,
    p_location_text: p.location_text ?? null,
    p_remote_join_url: p.remote_join_url ?? null,
    p_description: p.description ?? null,
    p_venue: p.venue ?? null,
    p_record_date: p.record_date ?? null,
  });
