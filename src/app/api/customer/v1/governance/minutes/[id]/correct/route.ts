...
import { createMinutesCorrectionSchema } from "@/lib/customer/governance-schema";
...
export async function POST(
  ...
  const { data: bodyJson, errorResponse } = await parseJsonWithLimit<unknown>(request, 64 * 1024);
  ...
  const parsed = createMinutesCorrectionSchema.safeParse(bodyJson);
  if (!parsed.success) {
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("create_minutes_correction_v1", {
    p_context_id: p.context_id,
    p_prior_minutes_id: minutesId,
    p_correction_reason: p.correction_reason,
    p_content_json: p.content_json,
  });
