...
import { recordMinutesSignatureSchema } from "@/lib/customer/governance-schema";
...
export async function POST(
  ...
  const parsed = recordMinutesSignatureSchema.safeParse(bodyJson);
  if (!parsed.success) {
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("record_minutes_signature_v1", {
    p_context_id: p.context_id,
    p_meeting_id: meetingId,
    p_signature_type: p.signature_type,
    p_signature_evidence_ref: p.signature_evidence_ref ?? null,
  });
