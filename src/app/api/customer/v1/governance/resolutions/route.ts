...
import { adoptResolutionSchema } from "@/lib/customer/governance-schema";
...
export async function POST(request: NextRequest) {
  ...
  const parsed = adoptResolutionSchema.safeParse(bodyJson);
  if (!parsed.success) {
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("adopt_resolution_v1", {
    p_context_id: p.context_id,
    p_meeting_id: p.meeting_id,
    p_agenda_item_id: p.agenda_item_id,
    p_resolution_no: p.resolution_no,
    p_title: p.title,
    p_text_body: p.text_body,
    p_responsible_actor: p.responsible_actor ?? null,
    p_due_date: p.due_date ?? null,
    p_financial_impact: p.financial_impact ?? null,
    p_maintenance_work_order_id: p.maintenance_work_order_id ?? null,
  });
