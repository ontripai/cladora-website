...
import { linkDocumentEntitySchema } from "@/lib/customer/documents-schema";
...
export async function POST(
  ...
  const parsed = linkDocumentEntitySchema.safeParse(bodyJson);
  if (!parsed.success) {
    return NextResponse.json(
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("link_document_entity_v1", {
    p_context_id: p.context_id,
    p_document_id: documentId,
    p_entity_type: p.entity_type,
    p_entity_id: p.entity_id,
    p_relation_type: p.relation_type,
  });
