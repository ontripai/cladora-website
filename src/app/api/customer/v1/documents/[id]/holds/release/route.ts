...
import { releaseLegalHoldSchema } from "@/lib/customer/documents-schema";
...
export async function POST(
  ...
  const parsed = releaseLegalHoldSchema.safeParse(bodyJson);
  if (!parsed.success) {
    return NextResponse.json(
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("release_legal_hold_v1", {
    p_context_id: p.context_id,
    p_document_id: documentId,
    p_release_reason: p.release_reason,
  });
