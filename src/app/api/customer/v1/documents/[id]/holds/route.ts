...
import { placeLegalHoldSchema } from "@/lib/customer/documents-schema";
...
export async function POST(
  ...
  const parsed = placeLegalHoldSchema.safeParse(bodyJson);
  if (!parsed.success) {
    return NextResponse.json(
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("place_legal_hold_v1", {
