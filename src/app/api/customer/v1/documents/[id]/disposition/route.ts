...
import { requestDispositionSchema } from "@/lib/customer/documents-schema";
...
export async function POST(
  ...
  const parsed = requestDispositionSchema.safeParse(bodyJson);
  if (!parsed.success) {
    return NextResponse.json(
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("request_document_disposition_v1", {
