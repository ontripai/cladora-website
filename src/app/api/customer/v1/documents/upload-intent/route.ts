...
import { createUploadIntentSchema } from "@/lib/customer/documents-schema";
...
export async function POST(request: NextRequest) {
  ...
  const parsed = createUploadIntentSchema.safeParse(bodyJson);
  if (!parsed.success) {
    return NextResponse.json(
  ...
  const p = parsed.data;
  const { data, error } = await (supabase.schema("customer_api") as any).rpc("create_upload_intent_v1", {
    p_context_id: p.context_id,
    p_document_id: p.document_id ?? null,
    p_filename: p.filename,
    p_declared_mime: p.declared_mime,
