...
import { renewOccupancyRequestSchema } from "@/lib/customer/occupancy-schema";
...
export async function POST(request: NextRequest) {
  ...
  const parsed = renewOccupancyRequestSchema.safeParse(rawBody);
  if (!parsed.success) {
  ...
  if (rpcError) {
    ...
    if (code === "22023" || msg.includes("invalid") || msg.includes("extend_beyond")) {
      return NextResponse.json({ error: { code: "BAD_REQUEST", message: "Invalid renewal parameters" } }, { status: 400, headers: HEADERS });
    }
