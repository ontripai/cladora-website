...
export async function GET(request: NextRequest) {
  ...
  const p = parsed.data;
  // Customer API Gateway: delegates to occupancy.get_customer_registry
  const { data, error: queryError } = await supabase.schema("customer_api").rpc("get_occupancy_registry_v1", {
