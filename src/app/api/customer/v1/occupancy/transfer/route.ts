...
import { transferOccupancyRequestSchema } from "@/lib/customer/occupancy-schema";
...
export async function POST(request: NextRequest) {
  ...
  const parsed = transferOccupancyRequestSchema.safeParse(rawBody);
  if (!parsed.success) {
  ...
  const { data, error: rpcError } = await supabase
    .schema("customer_api")
    .rpc("transfer_occupancy_v1", {
      p_context_id: b.context_id,
      p_occupancy_id: b.occupancy_id,
      p_to_unit_id: b.to_unit_id,
      p_effective_at: b.effective_at ?? null,
  ...
  if (rpcError) {
    ...
    if (code === "22023" || msg.includes("invalid") || msg.includes("cannot_transfer")) {
      return NextResponse.json({ error: { code: "BAD_REQUEST", message: "Invalid transfer parameters" } }, { status: 400, headers: HEADERS });
    }
    if (code === "P0002" || msg.includes("not_found")) {
      return NextResponse.json({ error: { code: "NOT_FOUND", message: "Occupancy or unit not found" } }, { status: 404, headers: HEADERS });
    }
