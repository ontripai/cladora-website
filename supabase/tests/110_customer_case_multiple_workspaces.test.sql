...
select is((select count(*)::integer from platform.customer_case_workspace_links where case_id=current_setting('case.multi.id')::uuid),2,'Denied links do not persist');
