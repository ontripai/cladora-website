...
const postAuthRoute = read('src/lib/auth/post-auth-route.ts');
...
const platformAuthContextMigration = read('supabase/migrations/20260923151939_platform_auth_context_gateway.sql');
...
check(authEmailPolicy.includes("invite: (lang) => [\`/\${lang}/invitation-continuation\`, \`/\${lang}/cases\`, \`/\${lang}/pilot-reviewer\`]") && authEmailPolicy.includes("recovery: (lang) => [\`/\${lang}/reset-password\`]"), 'verified Invite permits tokenless continuation or the customer case inbox while Recovery enters password reset');
...
check(invitationContinuationPage.includes(".schema('customer_api')") && invitationContinuationPage.includes('list_my_claimable_workspace_invitations_v1') && invitationContinuationPage.includes('supabase.auth.getClaims()'), 'invitation continuation lists only server-authorized choices through customer_api');
...
check(tokenlessClaimApi.includes(".schema('customer_api')") && tokenlessClaimApi.includes('claim_workspace_invitation_v1') && tokenlessClaimApi.includes("request.headers.get('origin') !== request.nextUrl.origin"), 'same-origin claim API delegates authorization through customer_api');
...
check(mfaPage.includes("assurance.currentLevel === 'aal2'") && mfaPage.includes("assurance.nextLevel !== 'aal2'") && mfaPage.includes("platformAccess ? 'platform_required' : 'customer_required'"), 'MFA routing separates satisfied, challengeable, and unenrolled sessions without a dashboard loop');
...
check(accountSecurity.includes("assurance.currentLevel !== 'aal2'") && accountSecurity.includes('window.location.replace(continueTo'), 'successful TOTP enrollment proves AAL2 before a fresh protected-route navigation');
...
check(postAuthRoute.includes(".rpc('has_platform_access_v1')") && postAuthRoute.includes('/mfa/setup?reason=platform_required'), 'post-auth routing identifies Platform users before selecting the MFA setup flow');
check(postAuthRoute.includes("assuranceResult.data.currentLevel !== 'aal2'") && postAuthRoute.includes('getPlatformOverviewRoute'), 'post-auth routing challenges enrolled factors and sends AAL2 Platform sessions to the control plane');
check(login.includes('window.location.replace(destination)'), 'login performs a fresh document navigation so protected layouts receive updated Auth cookies');
