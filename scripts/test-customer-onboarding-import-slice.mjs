...
for(const code of ["property","building","entrance","unit","party","ownership","occupancy","account","opening_gl","open_receivable","meter","meter_reading"])assert.match(pilotMigration,new RegExp(`'${code}'`));
assert.match(pilotMigration,/pg_advisory_xact_lock/);assert.match(pilotMigration,/finance\.assert_balanced/);assert.match(pilotMigration,/opening_ar_parity_failed/);assert.match(pilotMigration,/customer_advance_import_not_configured/);assert.match(pilotMigration,/security invoker set search_path=pg_catalog/i);assert.doesNotMatch(pilotMigration,/p1test|Tenant A/i);assert.match(pilotTest,/select plan\(40\)/);
for(const path of ["src/app/api/customer/v1/onboarding/templates/route.ts","src/app/api/customer/v1/onboarding/templates/[code]/download/route.ts","src/app/api/customer/v1/onboarding/imports/route.ts","src/app/api/customer/v1/onboarding/imports/[id]/sources/route.ts","src/app/api/customer/v1/onboarding/imports/[id]/[action]/route.ts"])assert.ok(existsSync(path),`${path} exists`);
